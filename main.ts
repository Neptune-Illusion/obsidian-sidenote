...
export interface Highlight {
    ...
    markType?: 'highlight' | 'underline' | 'strikethrough' | 'bold'; // Original Side Comments mark style
}
...
export interface CommentPluginSettings {
    ...
    enableSelectionToolbar: boolean; // Original Side Comments quick toolbar
    showHighlights: boolean; // Original Side Comments editor highlight toggle
    highlightOpacity: number; // Original Side Comments highlight opacity
    markdownFolder: string; // Original Side Comments markdown backup folder
    attachmentFolder: string; // Original Side Comments pasted image folder
    imageHashes: { [hash: string]: string }; // Original Side Comments pasted image dedupe map
}
...
export default class HighlightCommentsPlugin extends Plugin {
    ...
    async onload() {
        ...
        if ('registerHoverLinkSource' in this.app.workspace) {
            (this.app.workspace as any).registerHoverLinkSource(HOVER_SOURCE_SIDENOTE, {
                display: 'Side Note',
                defaultMod: true
            });
        }
        ...
        this.addCommand({
            ...
            editorCallback: (editor, view) => {
                ...
                else this.openFloatingCommentForActiveSelection('highlight');
            }
        });
        ...
        this.addCommand({
            ...
            editorCallback: (editor, view) => {
                ...
                else this.openFloatingCommentForActiveSelection('underline');
            }
        });
        ...
        this.addCommand({
            ...
            editorCallback: (editor, view) => {
                ...
                else this.openFloatingCommentForActiveSelection('strikethrough');
            }
        });
        ...
        this.addCommand({
            ...
            editorCallback: (editor, view) => {
                ...
                else this.openFloatingCommentForActiveSelection('bold');
            }
        });
        ...
        this.registerEvent(
            this.app.workspace.on('editor-menu', (menu, editor, view) => {
                if (editor.getSelection()) {
                    ...
                    menu.addItem((item) => {
                        item
                            .setTitle('Add comment (Highlight)')
                            .setIcon('highlighter')
                            .onClick(() => {
                                if (view instanceof MarkdownView) this.openFloatingCommentForSelection(editor, view, 'highlight');
                                else {
                    ...
                    menu.addItem((item) => {
                        item
                            .setTitle('Add comment (Underline)')
                            .setIcon('underline')
                            .onClick(() => {
                                if (view instanceof MarkdownView) this.openFloatingCommentForSelection(editor, view, 'underline');
                                else {
                                    const markdownView = this.app.workspace.getActiveViewOfType(MarkdownView);
                                    if (markdownView) this.openFloatingCommentForSelection(editor, markdownView, 'underline');
                                }
                    ...
                    menu.addItem((item) => {
                        item
                            .setTitle('Add comment (Strikethrough)')
                            .setIcon('strikethrough')
                            .onClick(() => {
                                if (view instanceof MarkdownView) this.openFloatingCommentForSelection(editor, view, 'strikethrough');
                                else {
                                    const markdownView = this.app.workspace.getActiveViewOfType(MarkdownView);
                                    if (markdownView) this.openFloatingCommentForSelection(editor, markdownView, 'strikethrough');
                                }
                    ...
                    menu.addItem((item) => {
                        item
                            .setTitle('Add comment (Bold)')
                            .setIcon('bold')
                            .onClick(() => {
                                if (view instanceof MarkdownView) this.openFloatingCommentForSelection(editor, view, 'bold');
                                else {
                                    const markdownView = this.app.workspace.getActiveViewOfType(MarkdownView);
                                    if (markdownView) this.openFloatingCommentForSelection(editor, markdownView, 'bold');
                                }
        ...
        return ViewPlugin.fromClass(class {
            ...
            updateToolbar() {
                ...
                if (!this.toolbarEl) {
                    ...
                    colorInput.value = colorOrDefault(plugin.settings.highlightColor);

                    const createButton = (icon: string, title: string, markType: Highlight['markType']) => {
                        const button = this.toolbarEl!.createEl('button', { cls: 'sidenote-toolbar-btn', attr: { title } });
                        setIcon(button, icon);
                        button.addEventListener('click', () => {
                            ...
                            if (editor && markdownView) {
                                plugin.openFloatingCommentForSelection(editor, markdownView, markType, colorInput.value);
                            }
                        });
                    };

                    createButton('highlighter', 'Add highlight comment', 'highlight');
    ...
    private openFloatingCommentForSelection(editor: Editor, view: MarkdownView, markType: Highlight['markType'] = 'highlight', initialColor?: string) {
        ...
        const coords = this.getEditorSelectionCoords(editor);
        new SidenoteFloatingCommentInput(
            this.app,
            this,
            async (comment, color) => {
                await this.addSidenoteCommentToSelection(editor, file, selection, comment, color, markType);
            },
            '',
            initialColor || colorOrDefault(this.settings.highlightColor),
            file.path
        ).open(coords);
    }

    private openFloatingCommentForActiveSelection(markType: Highlight['markType'] = 'highlight') {
        ...
        this.openFloatingCommentForSelection(editor, markdownView, markType);
    }

    private getEditorSelectionCoords(editor: Editor): DOMRect | { left: number; top: number; bottom: number } | null {
    ...
    private async addSidenoteCommentToSelection(editor: Editor, file: TFile, selectedText: string, comment: string, color?: string, markType: Highlight['markType'] = 'highlight') {
        ...
        if (matchingHighlight) {
            ...
            matchingHighlight.markType = markType;
            this.highlights.set(file.path, [...fileHighlights]);
        }
    ...
    private decorateReadingModeHighlights(containerEl: HTMLElement, sourcePath: string) {
        ...
        for (const { el: markEl, markType } of renderedMarks) {
            ...
            markEl.addClass(`sidenote-mark-${highlight.markType || 'highlight'}`);
            markEl.setAttr('data-highlight-id', highlight.id);
            if (highlight.color) {
                const rgb = this.hexToRgb(highlight.color);
                const opacity = this.settings.highlightOpacity ?? 0.2;
                markEl.style.setProperty('--sidenote-highlight-color', `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${opacity})`);
                markEl.style.setProperty('--sidenote-highlight-hover', `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${Math.min(opacity + 0.15, 1)})`);
                markEl.style.setProperty('--sidenote-highlight-border', `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${Math.min(opacity + 0.4, 1)})`);
            }
            markEl.addEventListener('mouseenter', (event) => {
                this.showReadingModeTooltip(markEl, highlight, event as MouseEvent);
            });
    ...
    private handleLivePreviewHighlightMouseover(event: MouseEvent) {
        ...
        const highlight = (this.highlights.get(file.path) || [])
            .find((item) => !item.isNativeComment && item.text.trim() === text);
        if (!highlight) return;

        const comments = highlight.footnoteContents?.filter((content) => content.trim() !== '') || [];
        if (comments.length === 0) return;

        this.showLivePreviewTooltip(markEl, comments, highlight.filePath, event);
    }

    private handleLivePreviewHighlightMouseout(event: MouseEvent) {
    ...
    private async showLivePreviewTooltip(targetEl: HTMLElement, comments: string[], sourcePath: string, event: MouseEvent) {
        this.hideLivePreviewTooltip();

        const tooltipEl = document.body.createDiv({ cls: 'sidenote-tooltip sidenote-live-preview-tooltip' });
        const contentEl = tooltipEl.createDiv({ cls: 'sidenote-tooltip-content markdown-rendered' });
        const component = new Component();
        component.load();
        this.register(() => component.unload());
    ...
    hexToRgb(color: string): { r: number; g: number; b: number } {
        ...
        if (!match) return { r: 255, g: 215, b: 0 };
        return {
    ...
    async createBackup(reason: string) {
        try {
            ...
            const backupsDir = '.obsidian/plugins/Side Comments/backups';
            try {
    ...
    async migrateBackupFilesToFolder() {
        try {
            const pluginDir = '.obsidian/plugins/Side Comments';
...
function colorOrDefault(color: string | undefined): string {
    return color || '#ffd700';
}
...
class SidenoteFloatingCommentInput {
    ...
    constructor(
        ...
        private onSubmit: (comment: string, color?: string) => void | Promise<void>,
        private initialComment: string = '',
        private initialColor: string = '#ffd700',
        private filePath: string = ''
    ) {}

    open(anchor?: DOMRect | { left: number; top: number; bottom: number } | null) {
        this.close();
        this.containerEl = document.body.createDiv({ cls: 'sidenote-floating-container' });

        const header = this.containerEl.createDiv({ cls: 'sidenote-floating-header' });
        header.createEl('span', { text: this.initialComment ? 'Edit Comment' : 'Add Comment', cls: 'sidenote-floating-title' });
        const closeButton = header.createEl('button', { cls: 'sidenote-floating-close clickable-icon' });
        setIcon(closeButton, 'x');
        closeButton.addEventListener('click', () => this.close());
        this.enableDrag(header);

        const inputWrapper = this.containerEl.createDiv({ cls: 'sidenote-floating-input-wrapper' });
        this.textareaEl = inputWrapper.createEl('textarea', {
            cls: 'sidenote-textarea',
            attr: { placeholder: 'Enter comment... (Paste images supported)' }
        });
        this.textareaEl.value = this.initialComment;
        this.textareaEl.addEventListener('paste', (event) => this.handlePaste(event));
        ...
        const footer = this.containerEl.createDiv({ cls: 'sidenote-floating-footer' });
        const colorWrap = footer.createDiv({ cls: 'sidenote-floating-color-picker' });
        colorWrap.createEl('span', { text: 'Color:', cls: 'sidenote-color-label' });
        this.colorInput = colorWrap.createEl('input', { type: 'color', cls: 'sidenote-color-input' });
        this.colorInput.value = this.initialColor;

        const cancelButton = footer.createEl('button', { text: 'Cancel', cls: 'sidenote-floating-cancel-btn' });
        cancelButton.addEventListener('click', () => this.close());
    ...
    private setPosition(anchor?: DOMRect | { left: number; top: number; bottom: number } | null) {
        ...
        if (top + height > window.innerHeight) top = anchor.top - height - 10;
    ...
    private enableDrag(handleEl: HTMLElement) {
        handleEl.addEventListener('mousedown', (event) => {
            ...
            const onMove = (moveEvent: MouseEvent) => {
                if (!this.containerEl) return;
                this.containerEl.style.left = `${startLeft + moveEvent.clientX - startX}px`;
                this.containerEl.style.top = `${startTop + moveEvent.clientY - startY}px`;
            };
            const onUp = () => {
    ...
    private async submit() {
        ...
        await this.onSubmit(comment, color);
        this.close();
    }

    private async handlePaste(event: ClipboardEvent) {
        if (!event.clipboardData || !this.textareaEl) return;
    ...
    private async saveImageAndInsertLink(file: File) {
        if (!this.textareaEl) return;
        try {
            const buffer = await file.arrayBuffer();
            const hash = await hashArrayBuffer(buffer);
            let imagePath = this.plugin.settings.imageHashes?.[hash];
            if (!imagePath || !(this.app.vault.getAbstractFileByPath(imagePath) instanceof TFile)) {
                ...
                this.plugin.settings.imageHashes = this.plugin.settings.imageHashes || {};
                this.plugin.settings.imageHashes[hash] = imagePath;
            ...
            let link = this.app.fileManager.generateMarkdownLink(imageFile, this.filePath || '/');
            if (!link.startsWith('!')) link = `!${link}`;
            const start = this.textareaEl.selectionStart;
            const end = this.textareaEl.selectionEnd;
            const value = this.textareaEl.value;
            this.textareaEl.value = `${value.substring(0, start)}${link}${value.substring(end)}`;
    ...
    private async createNewImage(buffer: ArrayBuffer, originalName: string): Promise<string> {
        const folder = normalizePath((this.plugin.settings.attachmentFolder || 'side-note-attachments').trim() || 'side-note-attachments');
        if (!(await this.app.vault.adapter.exists(folder))) {
            await this.app.vault.createFolder(folder);
        }

        const extension = originalName.split('.').pop() || 'png';
        const filename = `Pasted image ${window.moment().format('YYYYMMDDHHmmss')}.${extension}`;
        const path = await this.app.fileManager.getAvailablePathForAttachment(filename, folder);
